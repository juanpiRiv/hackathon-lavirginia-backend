...
Backend de validación de empaques de cápsulas de café. Expone una API REST que recibe una imagen del empaque, la analiza con un modelo de visión vía AI Gateway, y devuelve una decisión de control de calidad: **APROBADO** o **RECHAZADO**.
...
POST /api/validator/validate-package
  imagen + metadata opcional + output del modelo de polígonos (opcional)
        ↓
...
| `AI_GATEWAY_TIMEOUT_MS` | ❌ | Timeout en ms (default: `30000`) |
...
Campos:
  ...
  polygon_model_output  JSON string opcional con output del modelo de polígonos
```
...
curl -X POST http://localhost:3000/api/validator/validate-package \
  ...
  -F 'polygon_model_output={"raw":"output del modelo previo"}'
